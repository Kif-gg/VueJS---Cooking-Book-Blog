<template>
    <span class="pass-text">
        <div class="locked" @click="showPassword($event)" v-if="!showing">
        ...
        <div class="unlocked" @click="hidePassword($event)" v-else>
            <svg class="icon" xmlns="http://www.w3.org/2000/svg"
...
export default {
    ...
    methods: {
        showPassword(event) {
            event.currentTarget.parentElement.previousSibling.type = "text";
            this.showing = true
        },
        hidePassword(event) {
            event.currentTarget.parentElement.previousSibling.type = "password";
            this.showing = false
        }
    }
}
